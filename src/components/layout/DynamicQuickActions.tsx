...
const getQuickActionsByRoute = (pathname: string) => {
  switch (pathname) {
    case "/articles":
      return [
        { icon: FileText, label: "New Article", action: () => console.log("New Article") },
        { icon: Edit, label: "Draft Email", action: () => console.log("Draft Email") },
        { icon: Send, label: "Send Bulk Email", action: () => console.log("Send Bulk Email") },
...
export const DynamicQuickActions = () => {
  ...
  const quickActions = getQuickActionsByRoute(location.pathname);
    ...
    <div className="fixed right-6 top-20 z-50">
      ...
      >
        {quickActions.map((action, index) => (
          ...
          >
            {/* Label */}
            <div className="glass-card px-3 py-2 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity shadow-lg">
      ...
      <Button
        ...
        className={cn(
          ...
          "hover:scale-110 hover:shadow-2xl backdrop-blur-md",
